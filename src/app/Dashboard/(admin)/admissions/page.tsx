...
export default function AdminAdmissionManagementPage() {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      toast.success("Admission approved successfully! Student account created.", { id: toastId });
      setSelectedApp(null);
      fetchData();
    } catch (err: any) {
  ...
  const handleRejectSubmit = async () => {
    ...
    const toastId = toast.loading("Rejecting application & notifying student...");

    try {
      await admissionApi.rejectAdmission(selectedApp.id, rejectReason);
      toast.success("Application rejected and email notification sent.", { id: toastId });
      setIsRejectModalOpen(false);
      setSelectedApp(null);
      setRejectReason("");
      fetchData();
    } catch (err: any) {
      toast.error(err?.response?.data?.message || "Failed to reject admission", { id: toastId });
    } finally {
      setProcessingId(null);
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-emerald-800 to-teal-900 text-white p-6 rounded-2xl shadow-md">
        <div>
          ...
          <p className="text-xs text-emerald-100/80 mt-1">
            Review online application submissions, verify payments, approve student enrollment, or issue rejections.
          </p>
        </div>
        <div className="bg-emerald-950/60 px-4 py-2 rounded-xl border border-emerald-500/30 text-xs font-semibold">
          Total Applications: <span className="text-emerald-300 font-bold">{applications.length}</span>
        </div>
      </div>
      ...
      <Card className="border-border shadow-sm rounded-2xl">
        <CardContent className="p-4">
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
            ...
            {/* Submit Filter */}
            <Button type="submit" className="bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl font-bold">
              Apply Filter
            </Button>
          </form>
        </CardContent>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center space-y-2">
              ...
              <p className="text-sm font-semibold">No admission applications found!</p>
                ...
                <tbody className="divide-y divide-border">
                  {applications.map((app) => (
                    <tr key={app.id} className="hover:bg-muted/40 transition-colors">
                      <td className="p-3.5">
                        <span className="font-mono font-bold text-emerald-700 block">{app.applicationNo}</span>
                      ...
                      <td className="p-3.5">
                        <span className="font-bold block text-foreground">{app.studentName}</span>
                      ...
                      <td className="p-3.5">
                        <span className="font-semibold text-foreground">{app.class?.name || "N/A"}</span>
                      </td>
                      ...
                      <td className="p-3.5">
                        ...
                        <span className="text-[10px] text-muted-foreground">
                          {app.paymentMethod} • {app.amount} BDT ({app.senderPhone})
                        </span>
                      </td>
                      ...
                      <td className="p-3.5">
                        ...
                        {app.status === "PENDING" && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">
                            <FaSpinner className="animate-spin text-[9px]" /> Pending
                          </span>
                        )}
                        {app.status === "REJECTED" && (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background max-w-2xl w-full rounded-2xl shadow-2xl overflow-hidden border border-border max-h-[90vh] flex flex-col">
            <div className="p-4 bg-emerald-800 text-white flex justify-between items-center">
              <h2 className="font-bold text-sm">Application Details #{selectedApp.applicationNo}</h2>
              <button onClick={() => setSelectedApp(null)} className="text-white hover:opacity-80 font-bold">
                ✕
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3 bg-muted p-3 rounded-xl">
                <p><b>Applicant Name:</b> {selectedApp.studentName}</p>
                <p><b>Gender:</b> {selectedApp.gender}</p>
                <p><b>Date of Birth:</b> {new Date(selectedApp.dateOfBirth).toLocaleDateString()}</p>
                <p><b>Target Class:</b> {selectedApp.class?.name}</p>
                <p><b>Religion / Country:</b> {selectedApp.religion} / {selectedApp.country}</p>
                <p><b>Blood Group:</b> {selectedApp.bloodGroup || "N/A"}</p>
              </div>

              <div className="space-y-1">
                <h3 className="font-bold text-emerald-800 uppercase text-[11px] border-b pb-1">Parents & Contact</h3>
                <p><b>Father Name:</b> {selectedApp.fatherName} ({selectedApp.fatherOccupation || "N/A"})</p>
                <p><b>Mother Name:</b> {selectedApp.motherName} ({selectedApp.motherOccupation || "N/A"})</p>
                <p><b>Phone / Email:</b> {selectedApp.phone} | {selectedApp.email}</p>
                <p><b>Guardian Phone:</b> {selectedApp.guardianPhone}</p>
                <p><b>Present Address:</b> {selectedApp.presentAddress}</p>
              </div>

              <div className="space-y-1 bg-emerald-50 dark:bg-emerald-950/40 p-3 rounded-xl border border-emerald-200">
                <h3 className="font-bold text-emerald-900 uppercase text-[11px] border-b pb-1">Payment Verification</h3>
                <p><b>Method:</b> {selectedApp.paymentMethod}</p>
                <p><b>Sender Phone:</b> {selectedApp.senderPhone}</p>
                <p><b>TrxID:</b> <span className="font-mono font-bold text-emerald-700">{selectedApp.transactionId}</span></p>
                <p><b>Amount:</b> {selectedApp.amount} BDT</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background max-w-md w-full rounded-2xl shadow-2xl p-6 border border-border space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Please specify the reason for rejecting <b>{selectedApp.studentName}</b>'s application. An automated email notification will be sent.
            </p>
