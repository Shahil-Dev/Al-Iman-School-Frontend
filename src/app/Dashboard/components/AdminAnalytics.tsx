...
export default function AdminAnalytics() {
  ...
  if (!data)
    return (
      <div className="p-6 text-rose-500">Failed to load analytics data.</div>
    );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={`$${financials?.monthlyCollectedAmount}`}
          icon={DollarSign}
          color="bg-amber-500"
        />
      </div>
          ...
          <div className="mt-4 space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-slate-100">
              ...
              <span className="font-bold text-rose-600">
                ${financials?.totalDueAmount}
              </span>
            </div>
            <div className="flex justify-between items-center py-2">
              <span className="text-slate-600">Pending Payrolls:</span>
              <span className="font-bold text-amber-600">
                ${financials?.pendingPayrollAmount} (
                {financials?.pendingPayrollCount} Records)
              </span>
            </div>
          </div>
        </div>
